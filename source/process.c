...
int compareTime(realtime_t first_time, realtime_t second_time)
{
	...
	if (first_time.sec > second_time.sec){
		...
		if (second_time.msec > first_time.msec){
			return 1;
		}
		else{return 0;}
	}
...
int compareTimeEqual(realtime_t first_time, realtime_t second_time)
{
	...
	if (first_time.sec > second_time.sec){
		...
		if (first_time.msec > second_time.msec){
			return 0;
		}
		else{return 1;}
	}
...
static void startTimeSorting(process_t ** first_process, process_t * added_process) {
	process_t * current_process;
	if (*first_process == NULL) { //First element in the list
		added_process->next_process = NULL;
		*first_process = added_process;
	} else {
		current_process = *first_process;
		//Check if process should come before first process
		if(compareTime(added_process->start, current_process->start)){
			added_process->next_process = current_process;
			*first_process = added_process;
			return;
		}
		//Iterate to find correct spot to insert
...
static void DeadlineSorting(process_t ** first_process, process_t * added_process) {
	...
	if (*first_process == NULL) { //First element in the list
		...
		*first_process = added_process;
		...
		current_process = *first_process;
		//Check if process should come before first process
		if(compareTime(added_process->deadline, current_process->deadline)){
			added_process->next_process = current_process;
			*first_process = added_process;
			return;
		}
...
unsigned int * process_select(unsigned int * cursp)
{
	...
	if (first_process == 1){ //Don't change the first time with current_process=NULL, as it's when we start up
		...
		if (cursp==NULL) //The process has terminated - free the used space
		{
			...
			if(process_was_realtime){
				if(!(compareTime(ended_process->deadline, current_time))){
					//The deadline is before the current time
	...
	if(ready_rt_queue!=NULL){
		...
		while(compareTimeEqual(current_time, unready_rt_queue->start)) //Start time is greater than current time
		{
